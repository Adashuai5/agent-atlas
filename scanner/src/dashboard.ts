...
function compactResource(resource: AtlasSnapshot["resources"][number]) {
  ...
  return {
    ...
    identity: {
      ...
      hashAlgorithm: resource.identity.hashAlgorithm
    },
    lineage: {
      canonical: resource.lineage.canonical ? {
        sourceType: resource.lineage.canonical.sourceType,
        source: resource.lineage.canonical.source,
        sourceUrl: resource.lineage.canonical.sourceUrl,
      ...
      binding: resource.lineage.binding ? {
        ...
        viaPath: resource.lineage.binding.viaPath === resource.path ? undefined : resource.lineage.binding.viaPath
      } : null
    },
...
function compactSnapshot(
  ...
  const compactedResources = snapshot.resources
    .filter((resource) => (resource.type !== "plugin" || issueAssetIds.has(resource.id)) && (includeInventory || resource.effective || issueAssetIds.has(resource.id)))
    .map(compactResource);
...
<body>
  <main class="app">
    <header class="topbar">
      ...
      <div class="topActions"><span class="scanMeta"><span data-i18n="lastScan"></span> <time id="generatedAt" datetime="${escapeHtml(snapshot.generatedAt)}"></time></span><button id="showAiContext" class="topButton primary" data-i18n="aiContext"></button><button id="langToggle" class="topButton">中 / EN</button></div>
    </header>
  ...
  <script>
    ...
    function evidenceGapResources() { return visibleResources().filter((resource) => stateValue(resource.states?.enabled)==="unknown" || stateValue(resource.states?.loaded)==="unknown"); }
    ...
    function applyLanguage() {
      ...
      document.getElementById("generatedAt").textContent=new Intl.DateTimeFormat(language==="zh"?"zh-CN":"en-GB",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hour12:false,timeZoneName:"short"}).format(new Date(scopeSnapshots[0].generatedAt));
    ...
    function renderHero() {
      ...
      document.getElementById("heroLoaded").textContent=loaded+' / '+visible; document.getElementById("heroLoadedDetail").textContent='enabled '+enabled+' / '+visible+' · enabled ? '+enabledUnknown+' · loaded ? '+loadedUnknown;
    }
    function renderNavigation() { const hasEvidenceGap=evidenceGapResources().length>0; document.getElementById("navActionCount").textContent=actionableIssues().length+Number(hasEvidenceGap); document.getElementById("navRelationCount").textContent=relationIssues().length; document.getElementById("navPluginCount").textContent=globalPlugins.length; document.getElementById("navResourceCount").textContent=snapshot.resources.length; }
    function renderEvidenceCards() {
      ...
      const structuralRisk=countRelations("conflict")+countRelations("redundant")+countRelations("invalid"), actionable=actionableIssues().length, scanWarnings=snapshot.issues.filter((issue) => issue.kind==="scan-warning").length;
      const hardScanGap=ledger.installations.present.false+ledger.installations.valid.false+scanWarnings, unknownScanGap=ledger.installations.present.unknown+ledger.installations.valid.unknown;
      const cards=[
        ...
        {label:t("runtimeEvidence"),value:loaded+' / '+visible,foot:'enabled '+enabled+' / '+visible+' · enabled ? '+enabledUnknown+' · loaded ? '+loadedUnknown,tone:(enabledUnknown||loadedUnknown)?'var(--attention)':'var(--healthy)',view:"resources",mode:"unknown"},
        {label:t("scanCoverage"),value:ledger.installations.valid.true+' / '+ledger.installations.total,foot:t("present")+' '+ledger.installations.present.true+' · ? '+unknownScanGap+' · '+t("scanWarnings")+' '+scanWarnings,tone:hardScanGap?'var(--warning)':unknownScanGap?'var(--attention)':'var(--blue)',view:"resources",mode:"all"},
    ...
    function renderQueue() {
      const list=document.getElementById("queueList"), items=actionableIssues(), enabledUnknown=countVisibleState("enabled","unknown"), loadedUnknown=countVisibleState("loaded","unknown"), gapCount=evidenceGapResources().length;
      let html=items.map((issue,index) => { const tone=issue.severity==="warning"?["var(--warning)","var(--warning-soft)"]:["var(--attention)","var(--attention-soft)"]; const title=language==="zh"?issue.title:issue.titleEn, detail=language==="zh"?issue.detail:issue.detailEn, interactive=issue.assetIds.length>0; return '<button class="queueItem'+(interactive?'':' static')+'" '+(interactive?'data-queue="'+index+'"':'disabled aria-disabled="true"')+' style="--tone:'+tone[0]+';--soft:'+tone[1]+'"><span class="queueIcon">!</span><span><strong>'+esc(title)+'</strong><p>'+esc(detail)+'</p></span><span class="queueMeta">'+esc(issueKindLabel(issue.kind))+'</span></button>'; }).join("");
      if(gapCount) html+='<button class="queueItem" data-gap="true" style="--tone:var(--attention);--soft:var(--attention-soft)"><span class="queueIcon">?</span><span><strong>'+esc(t("evidenceGap"))+'</strong><p>'+esc(t("evidenceGapDetail"))+' '+esc(t("next"))+': '+esc(t("evidenceGapAction"))+'</p></span><span class="queueMeta">'+esc(gapCount)+' ?<small>enabled '+esc(enabledUnknown)+' · loaded '+esc(loadedUnknown)+'</small></span></button>';
    ...
    function renderPlugins() {
      ...
      const stateSelect=document.getElementById("pluginState"), selectedState=stateSelect.value||"all"; stateSelect.innerHTML=['all','bundled','installed','enabled','loaded','unknown'].map((key) => '<option value="'+key+'">'+esc(key==="all"?t("stateFilter")+': '+t("all"):t(key))+'</option>').join(""); stateSelect.value=selectedState;
    ...
    function renderResource(item) {
      const canonical=item.lineage?.canonical, installation=item.lineage?.installation, binding=item.lineage?.binding, storedReason=language==="zh"?item.reason:item.reasonEn, reason=storedReason||t(item.effective?"visibleReason":"inventoryReason"), canonicalText=canonical?.source||canonical?.sourceType||shortId(item.canonicalSourceId), pathText=item.identity?.isSymlink?item.path+' → '+(item.identity.realpath||'—'):item.path, viaPath=binding?.viaPath||item.path, hashText=(item.identity?.hashAlgorithm?item.identity.hashAlgorithm+':':'')+(item.identity?.contentHash||'—');
